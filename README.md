...
Built with **Claude Code (Opus 5)** as an implementation tool, under my direction, in a
single working session.

- **Direction and scope** — I owned the problem framing and every engineering decision
  that shaped what shipped: provider and endpoint strategy (one OpenAI-compatible
  interface so Gemini, OpenAI or a local server are interchangeable), how to size the run
  against hard quota limits, what to cut when those limits bit, and what was good enough
  to ship.
- **Implementation** — AI wrote the bulk of the code against that direction, and I
  reviewed it. The reviews that mattered were the ones that caught a metric which could
  not fail: an ask-extraction step positioned so `completeness` would have scored ~1.0 for
  every system forever, and a composite awarding full marks for `action_match` on live
  emails where no reference reply exists. Both are fixed and pinned by regression tests.
- **Dataset** — the 40-situation sampling grid, the 45-fact knowledge base with its
  agent-authority rules, and the 18 hand-authored adversarial cases with their named traps
  were authored during the session. Train/test rows are generated by `dataset/build.py`.
- **Verification** — 30 passing tests, a Docker image built and confirmed green, and every
  CLI command exercised end to end.

Using an AI coding tool well is itself an engineering skill: the value is in knowing what
to build, what to verify, and which output to reject. The two design flaws above were
caught by review, not by the model that wrote them.
